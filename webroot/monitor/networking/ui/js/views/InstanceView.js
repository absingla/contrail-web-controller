    ...
    var InstanceView = Backbone.View.extend({
        ...
        renderInstanceGraph: function(networkFQN, instanceUUID) {
            var topContainerElement = $('#' + ctwl.TOP_CONTENT_CONTAINER),
                connectedGraph = ctwu.getMNConnnectedGraphConfig(ctwc.get(ctwc.URL_NETWORK_CONNECTED_GRAPH, networkFQN), {fqName: networkFQN, instanceUUID: instanceUUID}, ':connected', ctwc.GRAPH_ELEMENT_INSTANCE),
                configGraph = ctwu.getMNConfigGraphConfig(ctwc.get(ctwc.URL_NETWORK_CONFIG_GRAPH, networkFQN), {fqName: networkFQN, instanceUUID: instanceUUID}, ':config', ctwc.GRAPH_ELEMENT_INSTANCE);
