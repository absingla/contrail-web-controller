...
function createStatRedisKey (req, query) {
    ...
    reqPayload.formModelAttrs = _.pick(reqPayload.formModelAttrs, function(value, key) {
        if (shouldCheck[key]) {
            return commonUtils.getValueByJsonPath(reqPayload, "formModelAttrs;" + key, null) === null;
        } else {
            return true;
        }
    });
    ...
    var redisKey = crypto.createHash("md5").update(md5Data).digest("hex");
...
function parseSLWhere(query, where, keywords) {
    var keywordsArray = keywords.split(","), keywordAndClause = [], i;
