...
var config = process.mainModule.exports.config,
    ...
    logutils = require(corePath + "/src/serverroot/utils/log.utils"),
    commonUtils = require(corePath + "/src/serverroot/utils/common.utils"),
    messages = require(corePath + "/src/serverroot/common/messages"),
    global = require(corePath + "/src/serverroot/common/global"),
    redisUtils = require(corePath + "/src/serverroot/utils/redis.utils"),
    redisReadStream = require("redis-rstream"),
    opApiServer = require(corePath + "/src/serverroot/common/opServer.api");
...
var redisClient =
    redisUtils.createRedisClient(redisServerPort, redisServerIP, global.QE_DFLT_REDIS_DB);
...
function saveData4Chart2Redis(queryId, dataJSON, plotFields) {
    var resultData = {},
        result, i, k, flowClassId, flowClassRecord, uniqueFlowClassArray = [], secTime,
        flowClassArray = [];
...
function parseSLQuery(reqQuery) {
    var msgQuery, fromTimeUTC, toTimeUTC, where,
        filters, table, level, category, moduleId,
        source, messageType, limit, keywords;

    reqQuery = parseFilterAndLimit(reqQuery);
...
function parseOTQuery(reqQuery) {
    var objTraceQuery, fromTimeUTC, toTimeUTC, where,
...
function parseSLWhere(query, where, keywords) {
    ...
    if (where !== null && where.trim() !== "") {
        var whereORArray = where.split(" OR "),
            whereORLength = whereORArray.length, newWhereOR,
...
function getOperatorCode(operator) {
    return OperatorCodeMap[operator] || -1;
}
...
function runNewQuery(req, res, queryId, reqQuery, appData) {
    var tableName = reqQuery.table, tableType = reqQuery.tableType,
        ...
        options = {
            queryId: reqQuery.queryId, pageSize: pageSize, counter: 0, status: "run",
